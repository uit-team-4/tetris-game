...
void Game::Draw(Font font) {
  ...
  switch (nextPiece.id) {
  case 3:
    nextPiece.Draw(225, 290);
    break;
  case 7:
    nextPiece.Draw(200, 270);
    break;
  default:
    nextPiece.Draw(245, 270);
    break;
  }
  if (gameOver) {
    ...
    for (size_t i = 0; i < highScores.size(); ++i) {
      ...
      DrawTextEx(font, scoreText, {scoreX, 300 + (float)i * 40}, 28, 2,
                 LIGHTGRAY);
    }
...
void Game::LockPiece() {
  std::vector<Position> tiles = currentPiece.GetCellPositions();
  for (const auto &item : tiles) {
    grid.SetCell(item.row, item.col, currentPiece.id);
  }
